        ...
        if (viewer != null) {
            ...
            if (query.getMember() != null && LoggedUser.isBroker()) {
                ...
                if (viewer.equals(member.getBroker())) {
                    ...
                    hql.append(" exists (select g.id from BrokerGroup g where doc in (select bg from g.brokerDocuments bg) and g = :group)) ");
                }
            }

            if (!searchingForABrokeredMemberDocuments) {
                hql.append(" and (doc.class = :memberType or exists (select g.id from Group g where doc in (select doc from g.documents doc) and g = :group)) ");
            }
