...
public class RestHelper {
    ...
    public static Pair<ServerErrorVO, Integer> resolveError(final Throwable t) {
        ...
        }else if (t instanceof NotEnoughCreditsException) {
            return Pair.of(handleNotEnoughCreditsException((NotEnoughCreditsException) t), HttpServletResponse.SC_BAD_REQUEST);
        }
        else {
    ...
    private static ServerErrorVO handleNotEnoughCreditsException(final NotEnoughCreditsException ex) {
        return new ServerErrorVO("NOT_ENOUGH_CREDIT", ex.getMessage());
    }
